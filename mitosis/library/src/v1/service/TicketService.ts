...
import {
    ...
    TicketComponentTextValue,
    TicketComponentNumberValue,
...
export class TicketService extends BaseService {
    ...
    public static initTicket(): TicketComponentForm {
        return new TicketComponentForm(
            "",
            new TicketComponentFormValue(
                new Map<LANGUAGES, string>(),
                [
                    ...
                    new TicketComponentForm(
                        ...
                        new TicketComponentFormValue(
                            ...
                            [0, 1, 2, 3, 4, 5, 6].map((idx) => {
                                return new TicketComponentSection(
                                    `${args.get('ticketId') ?? ''}-section-${idx}`,
                                    new TicketComponentSectionValue(
                                        new Map<LANGUAGES, string>([[
                                            LANGUAGES.EN_US,
                                            `testing section-${idx}`,
                                        ]]),
                                        [
                                            new TicketComponentLabel(
                                                `${args.get('ticketId') ?? ''}-section-label-${idx}`,

                                                new TicketComponentLabelValue(
                                                    new Map<LANGUAGES, string>([[
                                                        LANGUAGES.EN_US,
                                                        `testing label-${idx}`,
                                                    ]])
                                            ...
                                            new TicketComponentInputText(

                                                `${args.get('ticketId') ?? ''}-input-text-${idx}`,

                                                new TicketComponentTextValue(
                                                    new Map<LANGUAGES, string>([[
                                                        LANGUAGES.EN_US,
                                                        `testing input text-${idx}`,
                                                    ]]),
                                                    `value ${idx}`
                                                )
                                            ),
                                            new TicketComponentInputNumber(

                                                `${args.get('ticketId') ?? ''}-input-number-${idx}`,

                                                new TicketComponentNumberValue(
                                                    new Map<LANGUAGES, string>([[
                                                        LANGUAGES.EN_US,
                                                        `testing input number-${idx}`,
                                                    ]]),
                                                    idx
                                                )
                                            )
