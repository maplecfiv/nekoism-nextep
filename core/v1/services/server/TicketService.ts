...
import { Form as TicketComponentForm, Header as TicketComponentHeader, InputNumber as TicketComponentInputNumber, InputText as TicketComponentInputText, Label as TicketComponentLabel, Section as TicketComponentSection, TicketComponentFormValue, TicketComponentLabelValue, TicketComponentSectionValue, TicketComponentTextValue, TicketComponentNumberValue, FormElement, InputDate, InputTime, InputText, DropdownSelection, InputUrl, RadioSelection, Label, InputFile, TicketComponentSelectableValue, TicketComponentFileValue, InputNumber, TicketComponentUrlValue, CheckBoxSelection, Form, Section, TicketComponent, TicketComponentValueType } from "@nextep/core/v1/models/workflow/TicketComponent";
...
export class TicketService extends BaseService {
  ...
  override async processAction(action: ServiceAction, args: Map<string, any>): Promise<ActionResult> {
    switch (action) {
      case TicketService.LOAD_TICKET:
        this.setTicket(this.createDemoForm() as Form);
        return {
          isSuccess: true
        };
      default:
  ...
  private createDemoForm() {
    ...
    elements.push(new FormElement({
      parent: sectionBaseInformation.getElementId(), elementId: TicketService.createElementId(),
    ...
    elements.push(new FormElement({
      parent: sectionSpecialInformation.getElementId(), elementId: TicketService.createElementId(),
    ...
    //   columns: 2
    // }));
    return this.createTicketComponent(elements);
  }
  ...
  private createTicketComponent(elements: FormElement[], ticketComponentForm?: TicketComponentForm): TicketComponent<TicketComponentValueType> {
    ...
    elements.forEach((element) => {
      switch (element.getType()) {
        case Form.name:
          ...
          return this.createTicketComponent(elements.filter((_element) => { element.getElementId() != _element.getElementId() }), ticketComponentForm);
        case Section.name:
          ticketComponentForm?.getValue().getValue().push(new TicketComponentSection(element.getElementId(), new TicketComponentSectionValue(element.getLabelMap(), elements.filter((_element) => { return (_element.getParent() ?? '') === element.getElementId(); }).map((_element => {
            return this.createTicketComponent(elements,);
          })))));
          return ticketComponentForm;
        case InputText.name:
          return new InputText(element.getElementId(),
            new TicketComponentTextValue(element.getLabelMap(), "")
          );
        case InputFile.name:
          return new InputFile(element.getElementId(),
            new TicketComponentFileValue(element.getLabelMap(), "")
          );
        case InputNumber.name:
          return new InputNumber(element.getElementId(),
            new TicketComponentNumberValue(element.getLabelMap(), -1)
          );
        case InputUrl.name:
          return new InputUrl(element.getElementId(),
            new TicketComponentUrlValue(element.getLabelMap(), '')
          );
        case DropdownSelection.name:
          return new DropdownSelection(element.getElementId(),
            new TicketComponentSelectableValue(element.getLabelMap(), ''),
            {
              values: ((element.getOption() as { values: TicketComponentSelectableValue[] }).values as TicketComponentSelectableValue[])
            }
          );
        case CheckBoxSelection.name:
          return new DropdownSelection(element.getElementId(),
        ...
        case RadioSelection.name:
          return new RadioSelection(element.getElementId(),
            new TicketComponentSelectableValue(element.getLabelMap(), ''),
            {
              values: ((element.getOption() as { values: TicketComponentSelectableValue[] }).values as TicketComponentSelectableValue[])
            }
        ...
        case Label.name:
          return new Label(element.getElementId(),
            new TicketComponentLabelValue(element.getLabelMap())
          )
      }
    });
